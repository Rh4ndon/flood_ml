...
FINAL_PLOT = pd.read_csv('final_plot.csv', header=None)
FINAL_PLOT.columns = ['City', 'Latitude', 'Longitude', 'Temperature', 'Class']
@app.route('/map.html')
def weather_map():
    # Use precomputed data — no API calls!
    df_plot = FINAL_PLOT.copy()
    df_data = FINAL_DATA.copy()

    # Ensure both DataFrames have same number of rows and are aligned
    if len(df_plot) != len(df_data):
        # Fallback: truncate to shortest length
        min_len = min(len(df_plot), len(df_data))
    ...
    # Precipitation multiplier for Philippine context
    PRECIP_MULTIPLIER = 25

    for idx, row in df_plot.iterrows():
        try:
            # Get full weather features from final_data.csv (NO API CALL)
            full_weather = df_data.iloc[idx][['temp', 'max_temp', 'wind_speed', 'cloudcover', 'precip', 'humidity']].values.tolist()
            
            # Apply Philippine adjustment
            full_weather[4] *= PRECIP_MULTIPLIER  # precip = precip * 25
            ...
            fg_heatmap.add_child(
                folium.CircleMarker(
                    location=[row['Latitude'], row['Longitude']],
                    radius=full_weather[4] / 20,  # Scale radius by precip
                    color='blue',
                    fill=True,
                    fill_color='blue',
                    fill_opacity=0.3,
    ...
    # Build heatmap from static data
    heat_data = []
    for idx, row in df_plot.iterrows():
        try:
            precip = df_data.iloc[idx]['precip'] * PRECIP_MULTIPLIER
            heat_data.append([row['Latitude'], row['Longitude'], precip])
        except:
            continue
